...
(function () {
  ...
  /* Saves all the images created. */
  window.imgs = [];
  class ImageNew extends Image {
    constructor(...d) {
      super(...d);
      this.crossOrigin = "anonymous";
      window.imgs.push(this);
    }
  }
  ...
  if (!navigator.clipboard.readText) {
    navigator.clipboard.readText = function () {
      return new Promise((res) => {
        res(prompt("Paste text now.") || "");
      });
    };
  }

  /*XMLHttpRequest.prototype._open = XMLHttpRequest.prototype.open;
  XMLHttpRequest.prototype.open = function (...d) {
    if (d[1]?.includes("combined")) console.log(d);
    return this._open(...d);
  };*/
  ...
  const SETTINGS = {
    ...
    ...JSON.parse(localStorage.getItem("ninjaioutils") || "{}"),
  };
  const saveSettings = () => localStorage.setItem("ninjaioutils", JSON.stringify(SETTINGS));
  ...
  /*if (SETTINGS.texturePack) {
    const imgtest = setInterval(function () {
      window.imgs.forEach((i) => {
        if (i.src.includes("combined-min.png")) {
  ...
  if (window.SocialMenu)
    SocialMenu.prototype.maskInvitationList = function (scrollDist) {
      ...
      this.invites.forEach((i) => {
        ...
        if (!i.redReady) {
          ...
          i.on("mouseout", function () {
            i.tint = i.isRed ? Colors.red : 12303291;
          });
        }
      });
      ...
      this.listSearch.y =
        this.height -
        this.listSearch.height -
        pad / 2 -
        this.infoText.height -
        this.listContainer.y;
      this.invites.forEach((i) => {
        i.tint = 12303291;
        if (!filtered) return;
  ...
  let testing = setInterval(() => {
    ...
    function customTab() {
      function UtilTab() {
        ...
        this.pasteTexButton.x =
          this.marginLeft + this.texField.width + this.pasteTexButton.width / 4;
        ...
        this.pasteKeyButton.x =
          this.marginLeft + this.keyField.width + this.pasteKeyButton.width / 4;
